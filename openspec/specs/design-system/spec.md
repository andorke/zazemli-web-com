...
Система SHALL подключать ровно два семейства через `next/font/local` из файлов репозитория: voice-семейство (variable: opsz+wght, roman + italic; роль voice — заголовки, нарратив, латынь, italic-акценты; временно **Literata** как дублёр Newsreader, у которого нет кириллицы — финальное семейство утверждает Настя) и Commissioner (variable: wght 400–500; роль ui — кикеры, кнопки, навигация, мелкий UI). Субсеттинг MUST включать кириллицу и латиницу. Третье семейство вводить MUST NOT. CSS-переменные шрифтов SHALL именоваться по ролям: `--font-voice`, `--font-ui`.
...
- **THEN** заголовки и нарратив используют voice-семейство, кикеры/кнопки/навигация — Commissioner (ui)
