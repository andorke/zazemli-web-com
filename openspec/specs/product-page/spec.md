...
7 статических страниц товара (`/collectio/[slug]`) по прототипам `collectio-*.html`: единый шаблон блоков, типизированные и проверенные инвариантами данные SKU, SKU-цвет как декор без потери различимости earth-палитры материалов, размер-селектор с Ozon-CTA.
...
Страница SHALL устанавливать CSS-переменную `--sku` своего SKU и применять её только в декоративных элементах (kicker-акцент, бордер дневник-панели, RitualNote). Текст и кнопки SHALL использовать moss-ink/charcoal. Более одного SKU-цвета на странице MUST NOT быть. Исключение: MaterialDot в блоке состава остаётся earth-палитрой материала (color-system.md), не SKU-цветом.
