...
- [x] 2.2 Секции шаблона: hero (kicker, H1 «Заземли {растение}.», латынь, CTA, цены) и «Зачем именно эта земля» (+ SourceNote-аккордеон, колба-%, мост на /lab)
- [x] 2.3 Секции: состав с MaterialDot, «Что в боксе», care-блок с дневник-панелью, Ритуал (RitualNote)
- [x] 2.4 Buybar: клиентский размер-селектор (объём/цена/CTA-текст), состояние «Скоро на Ozon» при `null`, UTM `utm_content=sku00X` при URL
- [x] 2.5 Founder-quote («…а руками тянется к земле») + футер-мост «← Вся коллекция» на `/#collectio`
- [x] 2.6 SKU-цвет: переменная `--sku` на корне страницы, применение только в декоре; ds-lint-инвариант «один SKU-цвет на страницу»
