...
Колба-схема процентов в блоке «Зачем именно эта земля» SHALL рендериться без битых изображений: ассет колбы присутствует в репозитории либо заменён inline-SVG (ассета `assets/vial.png` в репо нет — сейчас картинка битая).
