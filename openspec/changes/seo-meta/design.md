...
4. **`next/image` в static export — `images.unoptimized`**: выгода волны 1 — lazy, decoding, дисциплина размеров/CLS и единый компонент; srcset/webp-оптимизация придёт с серверным окружением (VPS-roadmap) без правки вызовов. **Уточнение при реализации:** переводить оказалось нечего, кроме стекла колбы — единственного `<img>` в коде. `ImageSlot` контентных страниц изображения не рендерит вообще (иллюстрации ещё в производстве, слот — рамка с подписью и зарезервированной площадью), поэтому переводить его нечего до появления ассетов.
5. ~~**Колба — inline-SVG предпочтительнее** восстановления `vial.png`~~ — **отменено при реализации**: посылка неверна. `assets/vial.png` в коде не упоминается вообще, колба давно собрана компонентом `SoilVial` из `public/soil-vial.png` + CSS-маски `soil-vial-mask.png`; оба ассета в репозитории, битой картинки нет. Требование спеки («колба рендерится без 404») выполняется как есть, и переписывать композит (PNG-стекло под `mix-blend-multiply` поверх слоёв, обрезанных маской) на SVG значило бы перерисовывать штриховку и мерные засечки — потеря качества ради несуществующей проблемы. Вместо замены: регресс-тест на битые картинки в `out/` (`src/app/images.test.ts`).
