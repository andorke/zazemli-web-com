...
- [x] 5.1 TDD: `CookieBanner` («Принять» / «Только необходимые», выбор в localStorage, повторно не показывается)
- [x] 5.2 TDD: `lib/metrika.ts` + компонент `Metrika` (грузится только при согласии И `NEXT_PUBLIC_METRIKA_ID`; `reachGoal` — безопасный no-op без счётчика); цель `ozon-click` на кнопке Ozon
- [x] 5.3 E2E: первый визит — баннер виден, скрипт Метрики не загружен; после «Только необходимые» — скрыт и не грузится

## 6. Главная по Figma 185:2

- [x] 6.1 Выгрузить тексты всех текстовых нод Figma 185:2 в `content/home.ts` (типизированный `HomeContent`); тест: ключевые строки эталона присутствуют (кикеры II/IV, statement, CTA-строки)
- [x] 6.2 Секции 1–5: `Hero` (full-bleed слот, fluid clamp), `WhatsInBox` (chalk-фон, опись), `DifferentSoil` (кикер II, колбы-тройка слотами), `WhatSoilGives` (тёмная, photo-slot, accent rule buttercup), `SkuGallery` (кикер IV, 7 карточек из `content/sku.ts`, латынь italic, декоративная «7», линк «Открыть коллекцию →»)
- [x] 6.3 Секции 6–9: `Statement` (тёмная, ❦), `About`, `Teasers` (ГАЙД/ЛАБОРАТОРИЯ), `OzonCta` (CTA + подпись + кнопка через utm/заглушку)
- [x] 6.4 Сборка `/` из 9 секций в порядке эталона; изображения-слоты с aspect-ratio (без CLS); дудлы — `currentColor`
- [x] 6.5 Адаптив 360–1440: колонки складываются, нет горизонтального скролла; e2e-проверка на 360px
- [x] 6.6 Визуальная сверка с Figma 185:2 на 1440px (скриншот рядом с макетом), правки расхождений

## 7. Верификация и документы

- [x] 7.1 `npm run build` (static export) зелёный: в `out/` 5 страниц + 404; все unit и e2e тесты зелёные
- [x] 7.2 Lighthouse на главной (ориентир perf ≥90 до получения бюджета CDO); починить явные провалы
- [x] 7.3 Обновить документы проекта: CONTEXT.md (журнал + статус), ROADMAP.md (Фаза 0/1 чекбоксы), DEVELOPMENT.md (точные версии стека если разошлись), при необходимости ARCHITECTURE.md
