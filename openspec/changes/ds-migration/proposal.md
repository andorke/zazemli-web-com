...
- **BREAKING**: смена шрифтовых семейств — Unbounded/Spectral/Caveat выводятся полностью, вводятся Newsreader (300/400/500 + italic, роль voice) и Commissioner (400/500, роль ui), self-hosted woff2 с субсеттингом через `next/font/local`. Все font-переменные переименовываются во всём коде сразу; старые секции главной выглядят «переходно» до change `landing-redesign` (осознанный трейд-офф, контракт «3 шрифта max» сохраняется — семейств теперь 2).
