...
- [x] 5.1 Обновить ds-lint: запрет `unbounded|spectral|caveat` в `src/`, запрет `text-moss` без метки `ds-allow: moss-large`, запрет SKU-цвета на body-тексте/кнопках
  > Плюс скрипт `npm run ds-lint` (vitest по DS-инвариантам) — вход из сценария спеки.
- [x] 5.2 Прогнать ds-lint по всему `src/`, устранить нарушения (мелкий текст → moss-ink)
  > Единственное нарушение — Fleuron (text-moss): помечен `ds-allow: moss-large` (политика явно разрешает фльерону raw moss). KickerHeader ушёл на muted-foreground в 3.x.
...
- [x] 6.3 Обновить CONTEXT.md (журнал + текущая задача) и файл «вопросы Насте» (кириллица шрифтов — если возник, реквизиты ИП, дисклеймер весь сайт vs /lab)
  > Вопросы Насте (CONTEXT.md): Literata vs финальное voice-семейство, глиф ❦, дисклеймер весь сайт vs /lab, реквизиты ИП. Обновлены также DEVELOPMENT.md (стек шрифтов, пайплайн, ds-lint, запреты) и ARCHITECTURE.md (атомы, tokens v1.1.0).
