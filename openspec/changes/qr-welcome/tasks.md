...
- [x] 1.2 Инлайн show-once скрипт в `layout.tsx` (`<head>`, raw script): try/catch sessionStorage, класс `js-welcome` на `<html>` до первой отрисовки; unit-тест логики (первый заход / повторный / storage недоступен). Verify: `npm run test`, в браузере класс присутствует на первом заходе и отсутствует после навигации
- [x] 1.3 Базовые entrance-утилиты в `globals.css`: keyframes fade-rise (opacity от 0.1, translateY ≤10px), классы каскада с лесенкой задержек, всё под `html.js-welcome` и `@media (prefers-reduced-motion: no-preference)`. Verify: `npm run build`, при reduced-motion и без JS контент полностью видим
...
- [x] 3.1 Hero-каскад: kicker → h1 → подзаголовок → CTA; строки h1 в фиксированных спанах, масочный вылет `translateY(110%→0)` только с `lg`, на мобиле fade-rise (design D4, LCP-защита). Verify: `npm run build`, визуально на 1440 и 360
- [x] 3.2 Линия стадий 01–05: прорисовка `scaleX(0→1)` from left после hero-каскада. Verify: визуально, ds-lint
- [x] 3.3 Sticky-подсветка текущей стадии при скролле (IO по стадиям, подсветка через background-color/border-color токенами). Verify: скролл по 5 стадиям подсвечивает текущую, unit-тест хука
- [x] 3.4 Замер Lighthouse mobile /guide до/после: LCP регистрируется, деградация ≤0.2s, CLS < 0.1; при провале — отключить маску и на десктопе. Verify: отчёт замера в комментарии задачи
      <!-- Замер 2026-08-02, Lighthouse 12.8.2, preset mobile (4x CPU, Slow 4G), по 5 прогонов, медиана.
      Сборки: «до» — main (422c86d), «после» — night/qr-welcome (1ee81e9), обе отданы локальным статическим сервером.
      LCP 7956 → 7879 ms (−77 ms, деградации нет, порог ≤200 ms соблюдён); CLS 0.000 → 0.000 (< 0.1);
      FCP 1504 → 1655 ms (+151 ms); TBT 6 → 10 ms; performance score 75 → 75.
      LCP регистрируется в обеих сборках, но элемент сменился с h1 на подзаголовок: строки h1 обёрнуты
      в блочные `.welcome-line`, поэтому агрегат h1 распадается на строки. Значение LCP при этом не выросло.
      Маска остаётся включённой на lg — порог не превышен. -->
