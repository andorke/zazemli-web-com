...
## Текущая задача
Все 4 ночные ветки разобраны и смёржены в `main`: `privacy-page`, `diary-signup-form`, `product-pages`, `guide-lab`. Дальше: `/opsx:verify` → code review → `/opsx:sync` → `/opsx:archive` по каждому change; владельческие пункты (визуальная сверка 1440/360px, sign-off юриста на /privacy, аудит 6 линз по всем 10 страницам). Включение реального сбора email на /diary-signup — после РФ-бэкенда (Настя/инфра).
...
- [ ] `/opsx:verify` → `/opsx:sync` → `/opsx:archive` по privacy-page/diary-signup-form/product-pages/guide-lab
