...
- **[product-pages] SKU-цвет vs earth-палитра в составе** — design-решение 6 перечисляет «MaterialDot-заливки состава» среди мест `var(--sku)`, но прототип красит точки в earth-палитру (`--e-soil`…), а color-system держит earth «только в MaterialDot». Точки оставлены earth (иначе теряется различимость 10 материалов); `var(--sku)` — только kicker-номер, бордер дневник-панели, ритуал-приписка. Подтвердить трактовку.
