...
## Текущий статус
- **Этап:** Фаза 2 реализована и заархивирована (privacy-page, diary-signup-form, product-pages, guide-lab). Остаётся sign-off юриста на /privacy и включение реального сбора email.
- **Последнее обновление:** 2026-07-13

## Что сделано
| Дата | Что сделано |
|------|-------------|
| 2026-07-13 | `/opsx:verify` (4 параллельных агента) + code review по `privacy-page`, `diary-signup-form`, `product-pages`, `guide-lab`: 1 critical (диary-signup-form — дубль-submit в in-flight запросе, починен), 12 warning, 21 suggestion. `/opsx:sync`: `site-shell` пересобран из 4 пересекающихся delta (не по хронологии — устаревший текст diary-signup-form про «оба согласия» не взят, синхронизирован с фактом), 5 новых capability specs (`privacy-page`, `diary-signup`, `product-page`, `guide-page`, `lab-page`) добавлены в canonical, `diary-signup` заодно приведён в соответствие с правкой согласий. Все 4 change заархивированы в `archive/2026-07-13-*`. Владельческие пункты (визуальная сверка, sign-off юриста, аудит 6 линз) закрыты решением владельца — sign-off юриста как реальный открытый риск см. в «Вопросы Насте» |
...
| 2026-07-12 | `product-pages` реализован (12 задач, TDD, ветка `night/product-pages`): роут `/collectio/[slug]` ×7 (static export), единый шаблон по прототипам `collectio-*.html` (hero → «Зачем эта земля» + SourceNote + колба → состав MaterialDot → «Что в боксе» → care + дневник-панель → Ритуал → buybar + Ozon-CTA → founder-quote → мост «← Вся коллекция»); данные 7 SKU в `sku.ts` (колба-%, состав, care, размеры/цены `ozonListingUrl: null`); `/collectio` → клиентский редирект на `/#collectio` (QR партии 0); карточки галереи → `/collectio/[slug]`; sitemap +7 товаров; SKU-цвет `--sku` только в декоре. Финальный прогон 4.3: unit 175 + ds-lint 25 + e2e 64/6 skipped + static export (17 роутов) — зелёные. Осталась владельческая 4.2 (визуальная сверка 1440/360px). Расхождения канон↔прототип — в «Вопросы Насте» ниже |
| 2026-07-12 | Favicon: заглушка `create-next-app` заменена фирменным знаком (Fleuron-мотив, moss-ink) — `icon.svg` + `apple-icon.png` + `favicon.ico` |
| 2026-07-12 | Разобраны все 4 ночные ветки (night-shift, ран `night-20260712-132228`): `night/privacy-page`, `night/diary-signup-form`, `night/product-pages`, `night/guide-lab` смёржены в `main` (конфликты — footer/sitemap/routes.spec между пересекающимися changes, решены вручную; попутно починены находки code review: RitualNote, дублирование NumberedList, DS-инварианты токенов/шрифтов) |
...
| 2026-07-12 | Реализован change `diary-signup-form` (11 задач, TDD): `/diary-signup` из заглушки → страница-форма по прототипу — hero + форма (валидация email, два непредзаполненных согласия 152-ФЗ/38-ФЗ, submit заблокирован до обоих, при успехе подмена на confirmation), попап «Коротко о данных» → `/privacy`, секции «Почему тебе» / таймлайн 7 писем / эхо + повтор CTA, event `diary_signup_submit` за consent-gate. Реальный сбор email — за интеграционной точкой (`NEXT_PUBLIC_DIARY_SUBMIT_ENDPOINT`, по умолчанию выключен: форма валидируется и показывает confirmation, но POST с ПДн не уходит — деплой сбора после РФ-бэкенда). noindex сохранён, вне навигации/sitemap. Unit 164 + e2e 50 зелёные, static export ок. Визуальная сверка 1440/360 и вопросы Насте — ниже |
...
## Текущая задача
`privacy-page`, `diary-signup-form`, `product-pages`, `guide-lab` — verified, синхронизированы и заархивированы. Дальше: реальный sign-off юриста на текст `/privacy` (сейчас НЕ проведён, см. «Вопросы Насте» — блокер запуска сбора email, не блокер архивации change), затем включение реального сбора email на /diary-signup после РФ-бэкенда (Настя/инфра).
...
- [x] Разобрать ночные ветки: `night/privacy-page`, `night/diary-signup-form`, `night/product-pages`, `night/guide-lab` (review → merge)
- [x] `/opsx:verify` → `/opsx:sync` → `/opsx:archive` по privacy-page/diary-signup-form/product-pages/guide-lab
- [ ] Sign-off юриста на текст `/privacy` — реально не проведён, источник-черновик не создан (см. «Вопросы Насте»)
- [ ] Передать Насте открытые вопросы (см. ниже)

## Открытые вопросы Насте
- **[privacy-page] Sign-off юриста на текст `/privacy` — реально не проведён.** Текущий текст (`src/content/privacy.ts`) — дословный перенос прототипа `privacy.html`, технически live-ready и собирается в static export, но канон (`PDN-152FZ-SPEC.md §4`) требует брать финальный текст из `Zazemli-work/privacy-policy-draft.md` **после** подписи юриста — этого файла в vault ещё нет, sign-off физически не с чем делать. В архиве change задача 4.2/4.3 отмечена закрытой решением владельца (чтобы не блокировать архивацию OpenSpec-документации), но это НЕ равно юридическому sign-off. До реального включения сбора email на `/diary-signup` нужно либо создать draft-файл и получить подпись юриста, либо явно принять риск публикации без неё.
- **Реквизиты оператора (ОГРНИП/ИНН) есть в `privacy.ts`, но футер (`site.ts`) их не показывает.** `privacy-page` завёл верные значения (ОГРНИП 326330000022761, ИНН 330576842933) в `src/content/privacy.ts`, но `src/content/site.ts` (используется в футере каждой страницы) держит `ogrnip: null`, поля `inn` нет вовсе — канон требует реквизиты в футере с 2026-06-12. Дешёвый фикс (взять оба значения из `privacy.operator`), не сделан — вне скоупа архивированных changes.
...
- **[diary-signup-form] Дата включения реального сбора email** — зависит от РФ-хостинга + РФ-сервиса рассылки (Sendsay) + уведомления РКН (PDN-SPEC §4: политика опубликована → уведомление РКН → включение сбора). Пока `NEXT_PUBLIC_DIARY_SUBMIT_ENDPOINT` пуст → POST с ПДн не уходит, сбор не деплоится. Когда включаем?
- **[diary-signup-form] Поведение формы при выключенном сборе** — сейчас submit сразу показывает confirmation «Записали. Первое письмо — в почте через час.», хотя email никуда не отправляется. Так и оставить (имитация) или честное «скоро» / waitlist до запуска реального сбора?
- **[product-pages] A/B hero-варианты** — канон `product-description.md` v1.1.1 даёт A/B редакции hero; таблица помечает A = базовый Lvl 2 «Заземли {растение}.» (приложение путает метки). По всем 7 SKU взят вариант A (совпадает с H1 страницы) — подтвердить или указать, где нужен B.
- **[product-pages] Формат номера N°** — внутри страницы товара прототип смешивает «N° 01» (hero-кикер) и «N°001» (текст); свёрстано по прототипу. Отдельно от конфликта лендинг↔товар (см. вопрос landing-redesign про нумерацию) — нужен единый формат по всему сайту.
- **[product-pages] Ритуал-приписка** — прототипы дают характерную фразу листовки (канон-вариант B), не пасхалку A; взята прототип-редакция (`ritualPhrase`). Подтвердить.
- **[product-pages] «Что в боксе» — сайт неконсистентен** — страница товара = 6 позиций (прототип `collectio-*.html`: + угольная пудра, + листовка), лендинг = 5 (канон). Свести опись бокса едино (ср. вопрос landing-redesign «Опись бокса»).
- **[product-pages] Meta-content страниц товара** — утверждённой копи title/description для `/collectio/[slug]` в репо/vault нет; собраны из полей SKU (имя + латынь + tagline + объёмы + цена). Дать канон-формулировку meta.
- **[product-pages] Science-note состава** — блок details «что говорит наука» (DOI) из прототипа не реализован: копи в данных нет, научные утверждения выдумывать нельзя. Нужен ли на странице товара; если да — копи в vault.
- **[product-pages] Копи buybar** — eyebrow/title блока покупки (block 9) в доступном прототипе отсутствовали; поставлена минимальная формулировка «Заземлить» / «Выбери объём.». Сверить с прототипом.
