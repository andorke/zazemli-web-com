...
## Текущая задача
`ds-migration` закрыт полностью и смержен в main; `landing-redesign` реализован (11/11, ночь). Дальше: разбор ночных веток — review/merge `night/privacy-page` → `night/diary-signup-form`, решение по `night/product-pages` (11 коммитов на старой DS: пересобрать от новой базы или мигрировать), перезапуск `guide-lab` от нового main. Затем verify → sync → archive по каждому. Включение реального сбора email на /diary-signup — после РФ-бэкенда (Настя/инфра).
